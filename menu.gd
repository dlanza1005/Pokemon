...
func _ready() -> void:
	get_viewport().gui_focus_changed.connect(_on_focus_changed)# pass # Replace with function body.
	configure_focus()
...
func _unhandled_input(event):
	...
	if is_instance_valid(item) and event.is_action_pressed("select_key"):
		actioned.emit(item)
		#print(item)
...
func update_selection() -> void:
	...
	if is_instance_valid(item) and is_instance_valid(pointer) and visible:
		pointer.global_position = Vector2(global_position.x, item.global_position.y + item.size.y*.5)
